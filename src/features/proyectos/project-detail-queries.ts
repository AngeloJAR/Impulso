...
export async function getProyectoDetalle(
    ...
    const [proyectoResult, objetivosResult, tareasResult] = await Promise.all([
        ...
        supabase
            .from("objetivos")
            .select(
                "id, titulo, descripcion, estado, progreso, fecha_inicio, fecha_limite, created_at"
            )
            .eq("proyecto_id", id)
            .eq("user_id", user.id)
    ...
    return {
        ...
        tareas,
        metricas: {
            objetivosActivos: objetivos.filter(
                (objetivo) => objetivo.estado === "activo"
            ).length,
            objetivosCompletados: objetivos.filter(
                (objetivo) => objetivo.estado === "completado"
            ).length,
            tareasPendientes: tareas.filter((tarea) => tarea.estado !== "terminada")
                .length,
            tareasTerminadas: tareas.filter((tarea) => tarea.estado === "terminada")
                .length,
        },
    };
}